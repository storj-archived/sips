...
The renter will have a database of contacts and use the reputation algorithm described above to pick contacts, the renter will then send a `CLAIM` message with the format:
...
The farmer should then respond with an error to reject the contract or response with a signed contract and a token to authorize the client to upload data:
...
There is currently a "trusted keys" configuration for each farmer that defines which bridges to accept contracts, this configuration can be extended to include a bridge URL to notify changes to a farmers contact data. This directory can be discovered via an Ethereum contract as described in https://github.com/Storj/sips/pull/20
...
The farmer will first send an HTTP POST request to a bridge `/challenges` endpoint, the response will include:
```json
{
...
The farmer will then concatenate the `challenge + nonce` and generate a hash until the resulting value is less than the target hash using the [scrypt key derivation function](https://en.wikipedia.org/wiki/Scrypt). This specific parameters for the scrypt function are to be determined. This will then be included with further requests to create contacts along with a signature to verify updates in future requests. 

The signature should be an ECDSA signature of the following concatenated data:
```
<http_method><bridge-url><http-headers-sans-signature><http-body>
```

The farmer will then send an HTTP POST request to a bridge at `/contacts` with the headers with the challenge:
```
...
"x-node-signature: "fd62894f0074c1c6ee56c7ec5e28a0d686e122f388610ddb8a7229e98d2052bc3973331c6c56a5061e06407bca736d94b8c568523e90874aeb66581fcc1eabf6dc429256efafffd8c4"
"content-type": "application/json"
```

And with the JSON body:
```json
{
...
- The challenge and nonce meets the expected target difficulty and then process the request.
- The pubkey and node id correspend
- The timestamp is within an error threshold
- The signature is valid
- A contact does not already exist or the effects would be identical

These contact details can be updated in the future to include other preferences such as STORJ prices.

**Note**: A later improvement could be made to use an Ethereum contract as a farmer directory to share this state between decentralized applications, as similar to described in https://github.com/Storj/sips/pull/20

Farmers can later update their contact details by sending an HTTP UPDATE request to `/contacts/<node_id>` with these fields.:
```
{
  "address": "2001:0db8:0000:0042:0000:8a2e:0370:7334",
  "port": 4006,

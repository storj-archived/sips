...
The farmer will then concatenate the `challenge + nonce` and generate a hash until the resulting value is less than the target hash using the [scrypt key derivation function](https://en.wikipedia.org/wiki/Scrypt). This specific parameters for the scrypt function or to be determined. This will then be included with further requests to create contacts.

...
The farmer will then concatenate the `challenge + nonce` and generate a hash until the resulting value is less than the target hash using the [scrypt key derivation function](https://en.wikipedia.org/wiki/Scrypt). This specific parameters for the scrypt function or to be determined. This will then be included with further requests to create contacts. The farmer will then send an HTTP POST request to a bridge at `/contacts` with the headers:
```
{
...
```
{
  "nodeID": "86e529b1a0dd4429895afb030e5fd7cbc2a2fef2"
...
**Note**: An Ethereum contract could also be used to create a farmer directory to make sharing this information between bridges more efficient.

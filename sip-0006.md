...
The farmer will then concatenate the `challenge + nonce` and generate a hash until the resulting value is less than the target hash using the [scrypt key derivation function](https://en.wikipedia.org/wiki/Scrypt). This specific parameters for the scrypt function or to be determined. This will then be included with further requests to create contacts along with a signature to verify updates in future requests. The data that should be signed will be a concatenated timestamp, url, http method, http headers (sans signature header) and http body.

The farmer will then send an HTTP POST request to a bridge at `/contacts` with the headers:
```
...
A bridge will then verify that the challenge and nonce meets the expected target difficulty and then process the request. These contact details can be updated in the future to include other preferences such as STORJ prices.

**Note**: An later improvement could be made to use an Ethereum contract as a farmer directory to share this state between decentralized applications.
